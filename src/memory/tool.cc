...
//        memset(ptr, value, num);
    ...
    void math21_memory_malloc(void **p_data, size_t size) {
    ...
    cudaError_t math21_cuda_Free(void *ptr) {
        return cudaFree(ptr);
    }
    ...
    void math21_cuda_memcpy(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind) {
    ...
    void math21_cuda_EventCreate(cudaEvent_t *event) {
        cudaEventCreate(event);
    }

    void math21_cuda_EventRecord(cudaEvent_t event, cudaStream_t stream) {
        cudaEventRecord(event, stream);
    }

    void math21_cuda_EventSynchronize(cudaEvent_t event) {
        cudaEventSynchronize(event);
    }

    void math21_cuda_EventElapsedTime(float *ms, cudaEvent_t start, cudaEvent_t end) {
