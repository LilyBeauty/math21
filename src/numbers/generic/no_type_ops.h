...
//#pragma omp parallel for
    ...
    template<typename VecType>
    void math21_operator_merge(const VecType &x, const VecType &y, VecType &z) {
        NumN n1 = x.size();
        NumN n2 = y.size();
        NumN n = n1 + n2;
        z.setSize(n);
//#pragma omp parallel for
    ...
    template<typename VecType>
    void math21_operator_container_sub(const VecType &x, VecType &y, NumN from, NumN to = 0) {
    ...
    void math21_operator_container_replace(const Container<T> &A, Container<T> &B, const VecType &R, const T &x) {
