...
namespace math21 {
    ...
    void math21_print(std::ostream &io, const int &t, const char *s = 0);
    ...
    template<typename T>
    void m21log(std::string const &message, T b) {
#if defined(MATH21_DISABLE_LOG)
        return;
#endif
...
#include <android/log.h>
    ...
    static int m21printf(const char *format, ...) {
        va_list ap;
        va_start(ap, format);
        __android_log_vprint(ANDROID_LOG_DEBUG, "math21", format, ap);
        va_end(ap);
        return 0;
    }
...
#undef printf
#endif
#define printf(...) m21printf(__VA_ARGS__)
